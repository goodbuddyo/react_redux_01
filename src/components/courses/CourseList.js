...
const CourseList = ({ courses, onDeleteClick }) => (
  <>
    <Row className="testcols12">
      ...
      <Col xs="3" sm="3" md="7" className="so-courseListCol so-heading so-col so-col7" > Ref Link </Col>
    </Row>
    ...
    {courses.map(course => {
      return (
        ...
        <Row className="testcols12 so-courseRow" key={course.id}>
          ...
          <Col xs="3" sm="3" md="1" className="so-courseListCol so-col so-col2 btnRt" > 
            <a
          ...
          <Col xs="9" sm="9" md="5" className="so-courseListCol so-col so-col5" > Note: {course.refurl}</Col>
